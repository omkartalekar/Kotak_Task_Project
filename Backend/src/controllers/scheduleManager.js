...
exports.createSlots = async (req, res, next) => {
  try {
    const { startTime, endTime, duration, timezone = 'UTC' } = req.body;
...
exports.toggleSlot = async (req, res, next) => {
  try {
    ...
    if (slot.status === "BLOCKED") {
      ...
      // Block the slot
      slot.status = "BLOCKED";
      slot.blockReason = blockReason || "Blocked by provider";
      slot.blockedAt = new Date();
    }
    
    await slot.save();

    res.json({ message: "Slot updated", status: slot.status, slot });
  } catch (err) {
    next(err);
  }
};
